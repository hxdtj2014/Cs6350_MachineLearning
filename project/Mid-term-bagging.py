...
df = pd.read_csv(r'C:\Users/nanji\OneDrive\桌面\CS6350 Machine Learning'
                 r'\Project\Mid-term report\New folder/train_bin10.csv', header=None)
...
df_t = pd.read_csv(r'C:\Users/nanji\OneDrive\桌面\CS6350 Machine Learning'
                   r'\Project\Mid-term report\New folder/test_bin10.csv', header=None)
...
'''
# fetch the attribute names dictionary
Attr_dict = {}
...
# ==================================Bagging Algorithm==================================
def bagging_algorithm(total_iteration, df1, df2, num_examples):
    """
    :param total_iteration: the number of trees
    :param df1: training dataset
    :param df2: dataset that are used to serve as an input for predicting the labels
    :param num_examples: the amount of examples to draw
    ...
    for i in range(total_iteration):
        progress = progress + 1
        print(progress/total_iteration)
        ...
        trees = decision_tree(random_part, Attr_dict, 8, 0) # tree depth can be modified
    ...
    for j in range(num_trees):
        ...
        # make predictions
        predicts = prediction_dataframe(tree, df2)
        predict_matrix.append(predicts)
    hh = np.sum(predict_matrix, axis=0)
    final_prediction = [sgn_func(i) for i in hh]
    return final_prediction, classifier_trees, errors

"""
# =========================== test prediction =================================
tt = 50 # set the tree size to be 100 to reduce the computation time
results_test = bagging_algorithm(tt, df, df_t, 25000)
predict = results_test[0]
pp = results_test[0]
...
pp1.to_csv('C:/Users/nanji/OneDrive\桌面\CS6350 Machine Learning'
          '/Project/Mid-term report/bagging_predict_8.csv', index=False)
"""

# ==========================training error analysis==============================
tt = 50 # set the tree size to be 100 to reduce the computation time

results_train = bagging_algorithm(tt, df, df, 25000)
err_train = results_train[2]
...
b1 = np.dot(lo, a1)
acc_cum_train = b1*frac
iterations = [(i+1) for i in range(tt)]
print(iterations)
print(acc_cum_train)
plt.plot(iterations, acc_cum_train, label='training_error')
