...
def average_perceptron(data, epoch, y_s):
    """
    ...
    # define the average weight vector
    a = (np.zeros([1, w_length]))[0]
...
weight = average_perceptron(df, 4, y_s)
print(weight)


def average_perceptron_predict(data, w):
    """
    ...
    for k in range(len(data)):
        b = (data.loc[k, :]).values
        sgn = w.dot(b)
        predict_average.append(sgn_func(sgn))
...
weights = []
for i in range(1, 11):
    weight = average_perceptron(df, i, y_s)
    c = average_perceptron_predict(df_t, weight)
